...
class Basic_Web_Tests(unittest.TestCase):
    ...
    def test_required_elements_create(self): #OK
    ...
    def test_create_user_invalid_info(self): #OK
        ...
        for username in self.invalid_usernames:
            ...
            print(self.browser.title) #im literally right
            self.browser.save_screenshot(f"invalid_info1.png")
    ...
    def test_create_user_duplicate_username_email(self): #OK
    ...
    #------------------UPDATE tests-----------------
    def test_required_elements_update(self): #OK
        """user_details.html contains all required elements/id's"""
        for user in self.valid_users:
            self.browser.get(self.url)
            self.enter_and_submit_user_info(user["username"], user["password"], user["email"])
        ...
        print(f"{self.url}/{self.valid_users[2]['username']}")

        expected = self.user_details_update_requirements['title']
        actual = self.browser.title

        self.browser.save_screenshot(f"required_elements.png")
        self.assertEqual(actual, expected, f"The page title for user_details.html should be {expected}")
    ...
    def test_update_user_invalid_info(self): 
        ...
        for password in self.invalid_passwords:
            ...
            new_user = self.User_Model.get(username=username)
            self.assertEqual(new_user["status"], "error", "Invalid password should not be added to DB.")
    ...
    # #------------------DELETE tests-----------------
    def test_delete_user_exists(self): #ok
